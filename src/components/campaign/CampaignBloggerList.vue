    ...
    <v-data-table v-if="windowWidth >= 1024"
      ...
      <template v-slot:item="{ item }">
        <tr>
          ...
          <td class="campaign-bloggers-list__cell coverage">{{ item.influencer_reach_stories || 0 }}</td>
          <td class="campaign-bloggers-list__cell posts">{{ item.influencer_reach_post || 0 }}</td>
      ...
      <UiCard v-for="influencer in influencers" :key="influencer.id" class="ui-item-card">
        <div @click="toggleItem(influencer.id)" class="ui-item-card__header">
          <div class="ui-item-card__header-item">
              <a :href="networkList[influencer.influencer_network].link + influencer.influencer_network_account" target="_blank" class="icon-social" :class="[influencer.influencer_network]"></a>
