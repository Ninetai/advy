  ...
  <div class="overlay-bloger">
    ...
    <div v-else-if="item" class="popup popup_bloger">
      <h3 class="form__title form__title_create form__title_popup" v-if="item.network">
        <a @click.prevent="$emit('close-popup')" type="button"
          class="popup__link popup__link_comp">
        ...
        <a :href="networkList[item.network].link + item.network_account"
          target="_blank" class="name">
          @{{ item.network_account }}
        </a>
        <a :href="networkList[item.network].link + item.network_account"
          target="_blank" class="icon-social" :class="[item.network]"></a>
      </h3>
      <div v-if="selection">
        <div class="selection-help-text">
          <img v-if="item.profile_pic" :src="'https://app.advy.ai' + item.profile_pic" />
          <p v-if="item.description">{{ item.description }}</p>
          <p v-else><translate>Выберите, нравится или не нравится вам предложенный блогер. Понравившимся блогерам будет отправлено предложение разместить вашу рекламу.</translate></p>
        </div>
        ...
        <div class="selection-help-text-second" v-if="item.description">
          <p><translate>Выберите, нравится или не нравится вам предложенный блогер. Понравившимся блогерам будет отправлено предложение разместить вашу рекламу.</translate></p>
        </div>
        <div class="selection-progress-row">
          <div class="selection-progress-budget">
            ...
            <p><translate>Бюджет:</translate></p>
          ...
          <div class="selection-progress-influencer">
            <p><translate>Выбрано:</translate></p>
      ...
      <div class="popup-info">
        ...
        <div class="popup-info__block" v-if="item.reach_post_without_repost && item.reach_post_without_repost > 0">
          <p class="popup-info__head"><translate>Охват рекламных постов</translate></p>
        ...
        <div class="popup-info__block" v-if="item.channels_citation && item.channels_citation > 0">
          <p class="popup-info__head"><translate>Цитирования в каналах</translate></p>
        ...
        <div class="popup-info__block" v-if="item.channels_mentions && item.channels_mentions > 0">
          <p class="popup-info__head"><translate>Упоминания в каналах</translate></p>
        ...
        <div class="popup-info__block" v-if="item.channels_shares && item.channels_shares">
          <p class="popup-info__head"><translate>Репосты в каналах</translate></p>
        ...
        <div class="popup-info__block" v-if="influencer && item.desired_price && item.desired_price">
          <p class="popup-info__head"><translate>Предполагаемая цена</translate></p>
          <p class="popup-info__text">{{ !item.desired_price ? '&mdash;' : '$' + item.desired_price }}</p>
        </div>
        <div class="popup-info__block">
          <p class="popup-info__head"><translate>Бартер</translate></p>
      ...
      <div v-if="item.audience_age || item.audience_gender || item.audience_geo">
        <h3 class="form-document__head"><translate>Аудитория</translate></h3>
        <div class="popup-auditory">
          <ul class="tabs tabs_auditory clearfix">
            <li class="tabs__item tabs__item_task tabs__item_auditory" v-if="item.audience_age">
              <a class="tabs__link"
                :class="{active: selectedTab === 'age'}"
                @click.prevent="selectedTab = 'age'" href="#"><translate>Возраст</translate></a>
            </li>
            <li class="tabs__item tabs__item_task tabs__item_auditory" v-if="item.audience_gender">
              <a class="tabs__link"
                :class="{active: selectedTab === 'sex'}"
                @click.prevent="selectedTab = 'sex'" href="#"><translate>Пол</translate></a>
            </li>
            <li class="tabs__item tabs__item_task tabs__item_auditory" v-if="item.audience_geo">
              <a class="tabs__link"
                :class="{active: selectedTab === 'geo'}"
                @click.prevent="selectedTab = 'geo'" href="#"><translate>Гео</translate></a></li>
          </ul>
          <div class="task__tab">
            <div class="tabgroup">
              <div v-if="selectedTab === 'age'" class="tab-block">
                <UiValueProgress
                    v-for="(age, index) in item.audience_age"
                    :key="index"
                    :title="index"
                    :value="age"
                />
              ...
              <div v-if="selectedTab === 'sex'" class="tab-block">
                <UiValueProgress
                    v-for="(sex, index) in item.audience_gender"
                    :key="index"
                    :title="index"
                    :value="sex"
                />
              </div>
              <div v-if="selectedTab === 'geo'" class="tab-block">
                <UiValueProgress
                    v-for="(geo, index) in item.audience_geo"
                    :key="index"
                    :title="index"
                    :value="geo"
                />
      ...
      <div v-if="item.views_by_hour">
        <div class="statistics">
          <div class="statistics__wrap">
            <h3 class="descript__header descript__header_mrg"><translate>Статистика просмотров по часам</translate></h3>
          </div>
          <div class="graph">
      ...
      <div v-if="item.offers && item.offers.length">
        <h3 class="form-document__head"><translate>Рекламные посты Advy</translate></h3>
        <div class="popup-posts">
          <div class="popup-posts__block">
            <p class="popup-posts__head"><translate>Пост</translate></p>
            <p v-for="offer in item.offers"
              class="popup-posts__text popup-posts__text_post">№{{ offer.id || '&mdash;' }}</p>
          </div>
          <div class="popup-posts__block">
            <p class="popup-posts__head"><translate>CTR</translate></p>
            <p v-for="offer in item.offers"
              class="popup-posts__text">{{ offer.ctr ? offer.ctr + '%' : '&mdash;' }}</p>
          </div>
          <div class="popup-posts__block">
            <p class="popup-posts__head"><translate>Охват сторис</translate></p>
            <p v-for="offer in item.offers"
              class="popup-posts__text">{{ offer.reach_stories || '&mdash;' }}</p>
          </div>
          <div class="popup-posts__block">
            <p class="popup-posts__head"><translate>Охват постов</translate></p>
            <p v-for="offer in item.offers"
              class="popup-posts__text">{{ offer.reach_posts || '&mdash;' }}</p>
          </div>
        </div>
      </div>
      <div v-if="item.adverts && item.adverts.length">
        <div class="popup-info"></div>
        <h3 class="form-document__head"><translate>Рекламные посты в канале</translate></h3>
        <div class="popup-posts-channel">
          <div class="popup-posts-channel__block">
            <p class="popup-posts-channel__head"><translate>Пост</translate></p>
          </div>
          <div class="popup-posts-channel__block">
            <p class="popup-posts-channel__head"><translate>Просмотры</translate></p>
          </div>
          <div class="popup-posts-channel__block">
            <p class="popup-posts-channel__head"><translate>Прирост</translate></p>
          </div>
        </div>
